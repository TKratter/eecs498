...
def three_layer_convnet(x, params):
    ...
    scores = F.linear(input=flatten(x), weight=fc_w, bias=fc_b)
    ##############################################################################
    #                                 END OF YOUR CODE
    ##############################################################################
    return scores
...
class ThreeLayerConvNet(nn.Module):
    def __init__(self, in_channel, channel_1, channel_2, num_classes):
        ...
        self.linear = nn.Linear(in_features=channel_2 * H * W, out_features=num_classes)
...
def initialize_three_layer_conv_part4():
    ...
    model = nn.Sequential(OrderedDict([
        ('conv1', nn.Conv2d(in_channels=C, out_channels=channel_1, kernel_size=(kernel_size_1, kernel_size_1),
                            padding=pad_size_1)),
        ('relu1', nn.ReLU()),
        ('conv2', nn.Conv2d(in_channels=channel_1, out_channels=channel_2, kernel_size=(kernel_size_2, kernel_size_2),
                            padding=pad_size_2)),
        ('relu2', nn.ReLU()),
        ('flatten', Flatten()),
        ('linear', nn.Linear(in_features=channel_2 * H * W, out_features=num_classes))
    ]))
...
class PlainBlock(nn.Module):
    def __init__(self, Cin, Cout, downsample=False):
        ...
        self.net = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(in_channels=Cin, out_channels=Cout, kernel_size=(3, 3), stride=(stride, stride), padding=1),
            nn.BatchNorm2d(num_features=Cout),
            nn.ReLU(),
            nn.Conv2d(in_channels=Cout, out_channels=Cout, kernel_size=(3, 3), padding=1)
        )
        ############################################################################
        #                                 END OF YOUR CODE                         #
        ############################################################################
...
class ResidualBlock(nn.Module):
    def __init__(self, Cin, Cout, downsample=False):
        ...
        if (Cin == Cout) and (downsample is False):
            ...
            stride = 2 if downsample else 1
            self.shortcut = nn.Conv2d(in_channels=Cin, out_channels=Cout, kernel_size=(1, 1), stride=(stride, stride))
        ############################################################################
        #                                 END OF YOUR CODE                         #
        ############################################################################
...
class ResNet(nn.Module):
    def __init__(self, stage_args, Cin=3, block=ResidualBlock, num_classes=10):
        ...
        macro_blocks = [ResNetStage(num_in_channels, num_out_channels, num_blocks, do_downsample) for
                        (num_in_channels, num_out_channels, num_blocks, do_downsample) in stage_args]
    ...
    def forward(self, x):
        ...
        x = self.cnn(x)
        N, C, H, W = x.shape
        x = F.avg_pool2d(x, kernel_size=(H, W)).squeeze()
        scores = self.fc(x)
        ############################################################################
        #                                 END OF YOUR CODE                         #
        ############################################################################
...
class ResidualBottleneckBlock(nn.Module):
    def __init__(self, Cin, Cout, downsample=False):
        ...
        self.block = nn.Sequential(
            nn.BatchNorm2d(num_features=Cin),
            nn.ReLU(),
            nn.Conv2d(in_channels=Cin, out_channels=Cout//4, kernel_size=(1, 1), stride=(stride, stride)),
            nn.BatchNorm2d(num_features=Cout//4),
            nn.ReLU(),
            nn.Conv2d(in_channels=Cout//4, out_channels=Cout//4, kernel_size=(3, 3), padding=1),
            nn.BatchNorm2d(num_features=Cout // 4),
            nn.ReLU(),
            nn.Conv2d(in_channels=Cout // 4, out_channels=Cout, kernel_size=(1, 1))
        )
        if (Cin == Cout) and (downsample is False):
            self.shortcut = nn.Identity()
        else:
