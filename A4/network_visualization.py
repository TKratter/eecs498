...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
    ...
    for i in range(max_iter):
        ...
        if verbose:
            print(f'scores for iteration {i} are {float(loss.data.cpu().numpy())}')
        loss.backward()
        grad = X_adv.grad
        with torch.no_grad():
            X_adv += learning_rate * grad / torch.linalg.matrix_norm(grad).sum()
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    # Replace "pass" statement with your code
    scores = model(img)
    loss = scores[:, target_y] - l2_reg * torch.sum(img ** 2)
    loss.backward()
